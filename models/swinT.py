...
class PatchMerging(nn.Module):
    ...
    def _init_weights_layernorm(self,m):
        weight_attr = nn.init.constant_(m.weight, 1.0)
        bias_attr = nn.init.constant_(m.bias, 0)
...
class Mlp(nn.Module):
    ...
    def _init_weights(self,m):
        weight_attr = nn.init.constant_(m.bias, 0)
        bias_attr = nn.init.constant_(m.bias, 0)
        return weight_attr, bias_attr
...
def windows_partition(x, window_size):
    ...
    x = x.transpose(2,3)  # [bs,num_window,num_window,window_size,window_Size,C]
...
def windows_reverse(windows, window_size, H, W):
    ...
    x = x.transpose(2,3)  # [bs,num_window,window_size,num_window,window_size,C]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self,
        ...
        self.relative_position_bias_table = torch.create_parameter(
            shape=[(2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads],
            dtype='float32',
            default_initializer=torch.nn.initializer.TruncatedNormal(std=.02))
    ...
    def _init_weights(self,m):
        weight_attr = nn.init.constant_(m.weight, std=.02)
        bias_attr = nn.init.constant_(m.bias, 0)
        return weight_attr, bias_attr
    ...
    def forward(self, x, mask=None):
        ...
        attn = torch.matmul(q, k, transpose_y=True)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        w_attr_1, b_attr_1 = self.apply(self._init_weights_layernorm)
    ...
    def _init_weights_layernorm(self, m):
        weight_attr = nn.init.constant_(m.weight, 1)
        bias_attr = nn.init.constant_(m.bias, 0)
        return weight_attr, bias_attr
...
class SwinT(nn.Module):
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            x = block(x)
        print("afterblock_x:", x)
        if self.downsample is not None:
            x = self.downsample(x)
            # x = x.transpose((0, 2, 1))  # [B, out_channels, H//2 * W//2]
            x = x.transpose(1,2)  # [B, out_channels, H//2 * W//2]
