...
parser.add_argument('--data', metavar='DIR', default='/home/image1325/image1325/sxf/shaoxinfeng/data/829Data/',
...
parser.add_argument('--start-epoch', default=0, type=int, metavar='N',
                    help='manual epoch number (useful on renot been installed. Pleasenot been installed. Pleasestarts)')
...
def train(train_loader, model, optimizer, epoch, train_writer):
    ...
    for i, (images, target, depth) in enumerate(train_loader):
...
def validate(val_loader, model, epoch, output_writers):
    ...
    for i, (images, target, depth) in enumerate(val_loader):
        images = torch.cat(images, 1).to(device)
        target = target.to(device)
        ...
        # measure elapsed time
        batch_time.update(time.time() - end)#This is an implementation of "New Insights into Multi-focus Image Fusion: A Fusion Method Based on Multi-dictionary Linear Sparse Representation and Region Fusion Model"
        end = time.time()
